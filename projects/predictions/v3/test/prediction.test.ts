...
const MIN_BET_AMOUNT = ether("1"); // 1 CAKE
...
contract(
  ...
  ([operator, admin, owner, bullUser1, bullUser2, bullUser3, bearUser1, bearUser2, bearUser3]) => {
    ...
    beforeEach(async () => {
      ...
      // mint cake for test accounts
      const MintAmount = ether("100"); // 100 cake
      mockCake.mintTokens(MintAmount, { from: bullUser1 });
      mockCake.mintTokens(MintAmount, { from: bullUser2 });
      mockCake.mintTokens(MintAmount, { from: bullUser3 });
      mockCake.mintTokens(MintAmount, { from: bearUser1 });
      mockCake.mintTokens(MintAmount, { from: bearUser2 });
      mockCake.mintTokens(MintAmount, { from: bearUser3 });
      ...
      // approve cake amount for prediction contract
      const ApproveAmount = ether("10000000000000");
      mockCake.approve(prediction.address, ApproveAmount, { from: bullUser1 });
      mockCake.approve(prediction.address, ApproveAmount, { from: bullUser2 });
      mockCake.approve(prediction.address, ApproveAmount, { from: bullUser3 });
      mockCake.approve(prediction.address, ApproveAmount, { from: bearUser1 });
      mockCake.approve(prediction.address, ApproveAmount, { from: bearUser2 });
      mockCake.approve(prediction.address, ApproveAmount, { from: bearUser3 });
    });

    it("Initialize", async () => {
      assert.equal(await mockCake.balanceOf(prediction.address), 0);
    ...
    it("Should record data and user bets", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1.1"), { from: bullUser1 }); // 1.1 CAKE
      await prediction.betBull(currentEpoch, ether("1.2"), { from: bullUser2 }); // 1.2 CAKE
      await prediction.betBear(currentEpoch, ether("1.4"), { from: bearUser1 }); // 1.4 CAKE

      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), ether("3.7").toString()); // 3.7 CAKE
      assert.equal((await prediction.rounds(1)).totalAmount, ether("3.7").toString()); // 3.7 CAKE
      assert.equal((await prediction.rounds(1)).bullAmount, ether("2.3").toString()); // 2.3 CAKE
      assert.equal((await prediction.rounds(1)).bearAmount, ether("1.4").toString()); // 1.4 CAKE
      ...
      await prediction.betBull(currentEpoch, ether("2.1"), { from: bullUser1 }); // 2.1 CAKE
      await prediction.betBull(currentEpoch, ether("2.2"), { from: bullUser2 }); // 2.2 CAKE
      await prediction.betBear(currentEpoch, ether("2.4"), { from: bearUser1 }); // 2.4 CAKE

      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), ether("10.4").toString()); // 10.4 CAKE (3.7+6.7)
      assert.equal((await prediction.rounds(2)).totalAmount, ether("6.7").toString()); // 6.7 CAKE
      assert.equal((await prediction.rounds(2)).bullAmount, ether("4.3").toString()); // 4.3 CAKE
      assert.equal((await prediction.rounds(2)).bearAmount, ether("2.4").toString()); // 2.4 CAKE
      ...
      await prediction.betBull(currentEpoch, ether("3.1"), { from: bullUser1 }); // 3.1 CAKE
      await prediction.betBull(currentEpoch, ether("3.2"), { from: bullUser2 }); // 3.2 CAKE
      await prediction.betBear(currentEpoch, ether("3.4"), { from: bearUser1 }); // 3.4 CAKE

      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), ether("20.1").toString()); // 20.1 CAKE (3.7+6.7+9.7)
      assert.equal((await prediction.rounds(3)).totalAmount, ether("9.7").toString()); // 9.7 CAKE
      assert.equal((await prediction.rounds(3)).bullAmount, ether("6.3").toString()); // 6.3 CAKE
      assert.equal((await prediction.rounds(3)).bearAmount, ether("3.4").toString()); // 3.4 CAKE
      ...
      await prediction.betBull(currentEpoch, ether("4.1"), { from: bullUser1 }); // 4.1 CAKE
      await prediction.betBull(currentEpoch, ether("4.2"), { from: bullUser2 }); // 4.2 CAKE
      await prediction.betBear(currentEpoch, ether("4.4"), { from: bearUser1 }); // 4.4 CAKE

      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), ether("32.8").toString()); // 32.8 CAKE (3.7+6.7+9.7+12.7)
      assert.equal((await prediction.rounds(4)).totalAmount, ether("12.7").toString()); // 12.7 CAKE
      assert.equal((await prediction.rounds(4)).bullAmount, ether("8.3").toString()); // 8.3 CAKE
      assert.equal((await prediction.rounds(4)).bearAmount, ether("4.4").toString()); // 4.4 CAKE
    ...
    it("Should not allow multiple bets", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1"), { from: bullUser1 }); // Success
      await expectRevert(
        prediction.betBull(currentEpoch, ether("1"), { from: bullUser1 }),
        "Can only bet once per round"
      );
      await expectRevert(
        prediction.betBear(currentEpoch, ether("1"), { from: bullUser1 }),
        "Can only bet once per round"
      );
      await prediction.betBear(currentEpoch, ether("1"), { from: bearUser1 }); // Success
      await expectRevert(
        prediction.betBull(currentEpoch, ether("1"), { from: bearUser1 }),
        "Can only bet once per round"
      );
      await expectRevert(
        prediction.betBear(currentEpoch, ether("1"), { from: bearUser1 }),
        "Can only bet once per round"
      );
    ...
    it("Should not allow bets lesser than minimum bet amount", async () => {
      ...
      await expectRevert(
        prediction.betBull(currentEpoch, ether("0.5"), { from: bullUser1 }),
    ...
    it("Should record rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), ether("3.7").toString());
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal(
        (await mockCake.balanceOf(prediction.address)).toString(),
      ...
      assert.equal((await prediction.rounds(1)).rewardBaseCalAmount, ether("2.3").toString()); // 2.3 CAKE, Bull total
      assert.equal((await prediction.rounds(1)).rewardAmount, ether("3.7") * INITIAL_REWARD_RATE); // 3.33 CAKE, Total * rewardRate
      assert.equal((await prediction.rounds(2)).rewardBaseCalAmount, 0);
      assert.equal((await prediction.rounds(2)).rewardAmount, 0);
      assert.equal(await prediction.treasuryAmount(), ether("3.7") * INITIAL_TREASURY_RATE); // 3.7 CAKE, Total * treasuryRate
      assert.equal(
        (await mockCake.balanceOf(prediction.address)).toString(),
      ...
      assert.equal((await prediction.rounds(1)).rewardBaseCalAmount, ether("2.3").toString()); // 2.3 CAKE, Bull total
      assert.equal((await prediction.rounds(1)).rewardAmount, ether("3.7") * INITIAL_REWARD_RATE); // 3.33 CAKE, Total * rewardRate
      assert.equal((await prediction.rounds(2)).rewardBaseCalAmount, ether("2.4").toString()); // 2.4 CAKE, Bear total
      assert.equal((await prediction.rounds(2)).rewardAmount, ether("6.7") * INITIAL_REWARD_RATE); // 6.7 CAKE, Total * rewardRate
    ...
    it("Should claim rewards", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1"), { from: bullUser1 }); // 1 CAKE
      await prediction.betBull(currentEpoch, ether("2"), { from: bullUser2 }); // 2 CAKE
      await prediction.betBear(currentEpoch, ether("4"), { from: bearUser1 }); // 4 CAKE
      ...
      await prediction.betBull(currentEpoch, ether("21"), { from: bullUser1 }); // 21 CAKE
      await prediction.betBull(currentEpoch, ether("22"), { from: bullUser2 }); // 22 CAKE
      await prediction.betBear(currentEpoch, ether("24"), { from: bearUser1 }); // 24 CAKE
    ...
    it("Should multi claim rewards", async () => {
      ...
      // 4.2 = 2/3 * (7*0.9) + // 30.851162790697674418 = 22 / 43 * (67 * 0.9) = 35.051162790697674418 CAKE
    ...
    it("Should claim treasury rewards", async () => {
      let predictionCurrentCAKE = ether("0");
      assert.equal(await mockCake.balanceOf(prediction.address), 0);
      ...
      await prediction.betBull(currentEpoch, ether("2"), { from: bullUser2 }); // 2 CAKE
      await prediction.betBear(currentEpoch, ether("4"), { from: bearUser1 }); // 4 CAKE
      predictionCurrentCAKE = predictionCurrentCAKE.add(ether("7"));

      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), predictionCurrentCAKE.toString());
      ...
      await prediction.betBull(currentEpoch, ether("22"), { from: bullUser2 }); // 22 CAKE
      await prediction.betBear(currentEpoch, ether("24"), { from: bearUser1 }); // 24 CAKE
      predictionCurrentCAKE = predictionCurrentCAKE.add(ether("67"));
      ...
      await prediction.betBull(currentEpoch, ether("31"), { from: bullUser1 }); // 31 CAKE
      await prediction.betBull(currentEpoch, ether("32"), { from: bullUser2 }); // 32 CAKE
      await prediction.betBear(currentEpoch, ether("34"), { from: bearUser1 }); // 34 CAKE
      predictionCurrentCAKE = predictionCurrentCAKE.add(ether("97"));
      ...
      predictionCurrentCAKE = predictionCurrentCAKE.sub(ether("0.7"));
      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), predictionCurrentCAKE.toString());
      ...
      assert.equal(await prediction.treasuryAmount(), 0); // Empty
      predictionCurrentCAKE = predictionCurrentCAKE.sub(ether("16.4"));
      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), predictionCurrentCAKE.toString());
    });
    ...
    it("Should reject admin/owner functions when not admin/owner", async () => {
      ...
      await prediction.pause({ from: admin });
      await expectRevert(prediction.unpause({ from: bullUser1 }), "Not operator/admin");
    ...
    it("Should refund rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockCake.balanceOf(prediction.address)).toString(), ether("7").toString());
    ...
    it("Rejections for bet bulls/bears work as expected", async () => {
      // Epoch 0
      await expectRevert(prediction.betBull("0", ether("1"), { from: bullUser1 }), "Round not bettable");
      await expectRevert(prediction.betBear("0", ether("1"), { from: bullUser1 }), "Round not bettable");
      await expectRevert(prediction.betBull("1", ether("1"), { from: bullUser1 }), "Bet is too early/late");
      await expectRevert(prediction.betBear("1", ether("1"), { from: bullUser1 }), "Bet is too early/late");
      ...
      await prediction.genesisStartRound();
      currentEpoch = await prediction.currentEpoch();
      await expectRevert(prediction.betBull("1", ether("101"), { from: bullUser1 }), "transfer amount exceeds balance");
      await expectRevert(prediction.betBear("1", ether("101"), { from: bearUser1 }), "transfer amount exceeds balance");
      await expectRevert(prediction.betBull("2", ether("1"), { from: bullUser1 }), "Bet is too early/late");
      await expectRevert(prediction.betBear("2", ether("1"), { from: bullUser1 }), "Bet is too early/late");

      // Bets must be higher (or equal) than minBetAmount
      await expectRevert(
        prediction.betBear("1", ether("0.999999"), { from: bullUser1 }),
        "Bet amount must be greater than minBetAmount"
      );
      await expectRevert(
        prediction.betBull("1", ether("0.999999"), { from: bullUser1 }),
        "Bet amount must be greater than minBetAmount"
      );
    });
    ...
    it("Should prevent betting when paused", async () => {
      ...
      await expectRevert(prediction.betBull(currentEpoch, ether("1"), { from: bullUser1 }), "Pausable: paused");
      await expectRevert(prediction.betBear(currentEpoch, ether("1"), { from: bearUser1 }), "Pausable: paused");
    ...
    it("Should paginate user rounds", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1"), { from: bullUser1 });
      await prediction.betBull(currentEpoch, ether("1"), { from: bullUser2 });
      await prediction.betBear(currentEpoch, ether("1"), { from: bearUser1 });
    ...
    it("recoverToken function work as expected", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1"), { from: bullUser2 });
      await prediction.betBear(currentEpoch, ether("1"), { from: bearUser1 });
      assert.equal(await mockCake.balanceOf(prediction.address), ether("3").toString());
      await expectRevert(
        prediction.recoverToken(mockCake.address, ether("1"), { from: owner }),
        "Cannot be prediction token address"
      );

      let randomToken = await MockERC20.new("Random Token", "RT", _totalInitSupply);
      ...
      await randomToken.transfer(prediction.address, ether("66"), { from: bullUser1 });
      assert.equal(await randomToken.balanceOf(prediction.address), ether("66").toString());
      let tx = await prediction.recoverToken(randomToken.address, ether("66"), { from: owner });
      expectEvent(tx, "TokenRecovery", { token: randomToken.address, amount: ether("66") });
      assert.equal(await randomToken.balanceOf(prediction.address), 0);
